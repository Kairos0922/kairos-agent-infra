...
**内核是信推理,不信来源**:参照 Claude Code/Codex 是因为它们是**可查证的一流样本**,不是**权威**——采纳前必须(a)真去查、看到它们的真实做法,(b)确认其理由在本项目处境成立(它们为海量匿名用户的最大公约数优化,未必是你的处境)。来源只决定"这个做法值不值得优先一看",最终拍板永远是"哪个推理在我们这个具体项目里站得住"。官方无明确规范时(如 git 合并策略),不存在"信谁",退化为纯工程权衡,只按推理定。无推理的断言(不论出自何处、"大家都这么做")近乎零权重。

红线:严禁用"业界通用""与业界看齐"等模糊说辞掩盖未查证的臆断;用户已点名参照物(如 Claude Code)时,不得用未核实的理由推翻;亦不得把"参照 Claude Code"误用成"服从 Claude Code"——参照的前提是真查证并理解其理由,否则就是给臆断套权威外壳。
