...
- **ADR**:重大决策新增 `docs/adr/NNNN-*.md`,并同步更新 `docs/adr/README.md` 索引表。
- **memory 模块四件套**:改其一常须连带改其余——`memory-types.md`(数据模型/写入/检索/淘汰)、`retrieval.md`(检索/召回)、`api.md`(DTO/接口)、`tradeoffs.md`(取舍 + 来源)。
- **README 两处**:`docs/modules/memory/README.md`(结构/关键决策)、`docs/README.md`(导航/术语表/ADR 索引摘要)。
