...
高效、高质量、低成本地完成用户任务——一切实现围绕此。由此派生洁癖:
**非必要即不要**——任何功能、代码、文档,不是非有不可,就不加;走极简主义的
极致性能路线。新增任何东西前先过这把尺子:**它自足吗(不依赖臆测)?有真实消费者
或写明的下一步硬依赖吗?** 否则不加。删减时同样区分:砍投机代码,不砍写明的地基。
...
>
> 落地完成后、收尾前做一次**方案-实现一致性自检**:逐条比对产物与方案,重点查方案里写明「不需要/冗余/不改」的部分有没有被反手写进产物。
...
**查证在方案阶段完成**:格式/协议/分类这类不确定项,必须在报方案前查证收敛,禁止「先实现、被质疑后再回头查」——先干后查会让已落地产物被完整重写(配置格式 TOML/JSON 反复翻转、记忆分类轴反复推翻皆是此坑)。
...
6. **同会话内规范文档只读一次**:AGENTS/foundation/PROGRESS 等未变更的规范文件,一会话内读一次即留用,不因不确定就反复 Read——同会话重复读同一文件是纯 token 浪费。
...
5. ✅ 依赖方向由 Cargo crate 边界物理保证(下层不依赖上层;`providers` 为私有 mod,上层 crate 无法命名)。
...
依赖方向由 Cargo crate 边界在编译期强制,违反即编译失败(三契约均由 crate 边界与 `providers` mod 私有可见性物理保证,ADR 0021)。命名硬规则(ADR 0014)见 [`crates/AGENTS.md`](./crates/AGENTS.md)。详细约定见 [底座设计](./docs/foundation/foundation.md) 与 [整体架构](./docs/project/architecture.md)。
