...
- **项目层三处**:`docs/project/overview.md`(Non-goal)、`roadmap.md`(交付清单/路线)、`foundation/foundation.md`(目录/配置/契约测试)。
- **跨层交叉引用**:改契约/DTO 时核对 harness 消费方(context/tools/distill)与 memory 契约是否对齐;改分层命名时核对 `AGENTS.md` 命名硬规则、`pyproject.toml` import-linter 契约、`architecture.md`。
- **进度**:`PROGRESS.md`(任务项 + 变更记录)。

收尾两步,缺一不可:

1. 运行 `python3 tools/check_doc_links.py`,确认内部链接与锚点全绿。
...
> 判定"完成"的前提:任务列出的组件即便文件已存在(骨架占位),也须对照设计契约(如 model-gateway 要求的 `ProviderError.retryable`)与真实使用场景(可扩展/可配置)确认**真正满足**——"存在/能 import"不等于"落地完成"。
...
- **检查零容忍**:`ruff` 零告警、`mypy`/`pyright` 零错误——"零"是字面意思,不是"差不多"。
- **一次改动只做一件事**:不顺手改无关代码;无关的"看着不爽"另开任务。

**文档洁癖:**

- **三同步不留死角**:代码注释、文档、实现任一处改了,另两处同步,不留过时描述(过时的 `MemoryConfig` 字段即前车之鉴)。
- **改名/改决策全仓追到底**:术语、引用、索引、链接一处不漏,按"文档传播清单"逐项核对。
- **断链零容忍**:收尾跑 `tools/check_doc_links.py`,不留失效链接或锚点。
...
- 已落地决策见 [`docs/adr/`](./docs/adr/)(0001–0018),索引见 [ADR README](./docs/adr/README.md)。
...
1. **六层单向依赖**:`foundation`(L0)→ `modules`(L1)→ `harness`(L2)→ `assembly`(L3)→ `server`(L4)→ `cli`(L5),下层不知上层。由 import-linter 契约一(layers)强制。
2. **L1 模块间零依赖**:每个 infra 模块只依赖底座(`foundation/`)和自己,**不依赖其他模块内部**。跨模块编排只发生在 harness 层。由契约二(independence)强制。
3. **领域逻辑不依赖具体实现**:`modules/memory/` 的领域逻辑(`store`、`kinds/`、`retrieval/searcher`)**不得 `import lancedb`、不得 import 自己的 `providers/`**。只依赖模块内的 `contracts/` 抽象,实现由组装根 factory 配置注入(ADR 0011)。
4. **harness 只依赖各模块 contracts、禁触 providers**:harness 是唯一跨模块编排层,只 import 各模块的 `contracts/`,不碰任何 `providers/`。由契约三(forbidden)强制。
5. **租户隔离是不变量**:所有涉及租户数据的接口首参 `ctx: TenantContext`(ADR 0012,禁 contextvar);`TenantContext` 只在 server 认证中间件构造(ADR 0010);记忆按 `{tenant_id}__{kind}` 物理分表 + 表内 `owner_id` 过滤(ADR 0013);缺作用域 fail-closed(ADR 0009)。
6. **避免过度设计(YAGNI)**:只为当前阶段真正需要的东西设计。共享抽象按需上提——出现第二个消费者且确有复用需求时才上提到底座(ADR 0003/0015),不提前预测。
7. **底层异常不外泄**:`lancedb` / `openai` / `mcp` 等原始异常必须在 provider 层封装成 `ProviderError`,不穿透到上层。
8. **对外 API 一律 async**;纯 CPU 计算(分词、RRF)保持同步。

依赖方向规则由 `import-linter` 三契约在 CI 强制,违反会导致检查失败。

详细约定见 [底座设计](./docs/foundation/foundation.md) 与 [整体架构](./docs/project/architecture.md)。

## 命名硬规则(ADR 0014,冻结)

1. **包名/模块名**:小写 snake_case,单数(目录 `contracts/` 例外,与既有 memory 模块一致)。
2. **L1 模块固定骨架**:`contracts/`、`providers/`、`factory.py` 三件为强制命名,不得改叫 `interfaces/`、`impl/` 等同义词;模块特有领域逻辑目录自由命名但须在模块 README 声明。
3. **契约类命名**:用能力名词(`ChatModel`、`ToolRegistry`、`SessionStore`),**不加 `Abstract`/`Base`/`I` 前缀**;实现类 = `<技术名><契约名>`(`LanceDbVectorStore`、`OpenAiCompatChatModel`)。
4. **DTO 命名**:`<领域名词>` 或 `<动词>Request/Result`,统一 **Pydantic v2**,**禁止裸 dict 跨层传递**。
5. **事件命名**:`AgentEvent` 的 type 用 snake_case(`step_started`、`run_finished`),在 [protocol/agent-events](./docs/protocol/agent-events.md) 冻结枚举。
6. **禁用词表**:`util`/`utils`/`common`/`helper` 作包名或模块名**全仓禁用**(低内聚温床);确有共享逻辑,按语义命名并归入正确层。
7. **改名即全仓**:任何命名变更走"文档传播清单",`grep` 零残留。

## 工程化基线

- 包管理 / 依赖:`uv`,依赖锁定版本(不用开放区间)。
- 格式化 + lint:`ruff`。
- 类型检查:`mypy`(strict)/ `pyright`。
- 测试:`pytest` + `pytest-asyncio` + `pytest-cov`。
- 依赖方向:`import-linter`(六层三契约)。
...
uv run lint-imports           # 依赖方向检查(import-linter)
uv run pytest --cov           # 测试 + 覆盖率
python3 tools/check_doc_links.py   # 文档内部链接与锚点检查(收尾必跑)
```

> 完整验证链(提交前必过):`ruff format --check` → `ruff check` → `mypy` → `lint-imports` → `pytest --cov`,与 CI(`.github/workflows/ci.yml`)一致。
