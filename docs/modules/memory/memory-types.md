...
| **写入触发** | 显式写入 + 从对话抽取 | 显著性门控下追加会话/事件 | 异步从 trace 提炼 |
...
```python
# modules/memory/procedural/trace_schema.py(草案)
...
> **为什么先规则后 LLM?** LLM 抽取是这条链路最贵的一步。用零成本规则先挡掉大量不值得处理的 trace,只把"可能有价值"的喂给 LLM,控成本。与语义记忆"先去重再写"是同一个成本意识。
...
```python
# modules/memory/procedural/distiller.py(接口草案)
class ExperienceDistiller:
    def __init__(self, llm: LLMProvider): ...  # LLM 也走 Provider 抽象

    async def distill(self, trace: ExecutionTrace) -> list[ProceduralMemory]:
...
**⑥ 去重 + 写入。** 与语义记忆同样的去重:新经验先向量检索,高度相似则合并,否则新增。走统一写入管线。

**⑦ 反馈回路(轻量)。** 一条经验被检索命中、被 Agent 采用且本次执行成功时,上层回调 `reinforce(experience_id, success=True)`,提升 `effectiveness`/`reuse_count`;反之降低。**这是程序记忆区别于另两类的关键——它会"越用越准"。** 本阶段提供回调 + 简单指数移动平均,复杂信用分配列为扩展。
...
| 从对话抽取语义记忆 | ✅ 钩子,默认手动触发 | 自动抽取 + 冲突消解 |
| trace 提炼程序记忆 | ✅ 规则门控 + 单次 LLM 抽取 | 自动分段、反思循环、信用分配 |
...
> 三类记忆的设计取舍依据见 [ADR 0006](../../adr/0006-memory-classification-by-cognitive-function.md)(分类轴)、[ADR 0004](../../adr/0004-no-knowledge-graph-mvp.md)(不做图)与 [ADR 0005](../../adr/0005-decay-ranking-conflict-deletion.md)(衰减/删除分离);效果由 [benchmark](../benchmark/README.md) 验证。
