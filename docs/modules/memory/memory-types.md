...
> - **按认知功能分类**(ADR 0006):先按生命周期切出**工作记忆**(归应用/适配层,不是 infra 存储)vs **长期记忆**;长期记忆内部按认知功能分三类——**语义 semantic**(关于用户的事实/偏好)、**情景 episodic**(发生过的对话/事件)、**程序 procedural**(从执行学到的策略)。
...
> - **机制 vs 策略**(ADR 0007):记忆模块只提供**机制**(存、检索、去重、衰减排序);"**何时存、何时召回、什么值得记**"是**策略**,留在模块外(应用/适配层)。写入分 kind 不每轮无差别写;召回选择性、带门控,不每轮全量。
> - **procedural 评估/提炼解耦**(ADR 0008):trace 的采集/评估/提炼是**记忆模块之外**的独立关注点;模块对 procedural 只接收"已提炼、已评估的经验"。
> - **作用域与隔离**(ADR 0009):单用户是多用户的退化情形,共用一套作用域(`namespace` 租户边界 + `owner_id` 实体)。隔离是机制(每次检索强制带作用域、缺则拒绝);跨用户共享是策略(留阶段二)。三类记忆 owner 语义不同(见下)。
>
...
─────────────────       ──────────────────────────────────
工作记忆 working    ──→  归应用/适配层(context 窗口 + 压缩),infra 不存
...
- **归属(用户/会话/agent)不是分类轴,是字段 `owner_id`**;**垂直/场景**靠 `namespace`/`tags`/应用层表达(见下文);**生命周期/衰减**是每类的策略,不是主轴。
- **工作记忆(context 压缩)归应用层**,不是 infra 的存储 kind——上下文窗口的保留与压缩是应用层职责。这一刀把"短期 vs 长期"之争解决在源头:短期上下文 = 工作记忆 = 应用层管;真正需要 infra 持久化的是"离开 context 就会丢"的长期记忆。
...
- **垂直差异只体现在"写什么内容"和 benchmark 场景,不体现在 schema**:个人助理记用户偏好,教育助手记教师的教学偏好/学段学科/班级观察——都是 `semantic` 里的原子事实,**同一套 schema、同一个 owner、同一个库共存**,靠 `category` / `tags` / `namespace` 区分。
...
| **写入触发** | 显式写入 + 信息**定型**时异步抽取 | 显著性门控下追加(保持原始) | 模块外评估/提炼后写入已提炼经验 |
| **owner** | 用户 | 用户(可跨会话) | Agent |
| **作用域/可共享性** | `owner_id=user_id`,namespace 内私有 | `owner_id=user_id`,namespace 内私有 | owner **二选一**:私有 `f(agent_id,user_id)` / 全局技能 `agent_id`;本阶段仅私有 |
...
> **工作记忆去哪了?** context 窗口的保留与压缩是**应用/适配层**的职责(ADR 0006),不是 infra 的存储 kind。infra 只负责"离开 context 会丢"的长期记忆。情景记忆(episodic)是工作缓冲的**无损可检索后备**:压缩把细节丢了,还能从 episodic 精确捞回——但它**不等于**工作缓冲本身。
...
记忆效果取决于两件事——**高质量的记忆数据** + **召回的时机**。但二者是**策略**(依赖业务与上下文的判断),不是**机制**(确定的存取能力)。[ADR 0007](../../adr/0007-memory-mechanism-vs-policy-timing.md) 据此划界:记忆模块提供机制(`remember` / `extract_semantic` 钩子 / 经验写入口 / `Searcher` / `RecallRouter`),"何时存、何时召回"由模块外(应用/适配层)决定。

**写入时机(分 kind,绝不每轮无差别写)**:

| 记忆 | 写入时机 | 形态 |
|------|---------|------|
| semantic | 信息**定型**时(话题切换 / 会话结束 / 显式确认),**异步**抽取 | 上层在合适时机触发,不在对话每轮跑 |
| episodic | 近实时追加,过**显著性门控**,**保持相对原始、不做重抽取** | 适配层在会话推进时写入,门控打 `salience` |
| procedural | 任务边界后、**经评估**才落库(评估/提炼解耦,见 [ADR 0008](../../adr/0008-procedural-evaluation-decoupling.md)) | 模块只收"已提炼经验" |

> 高质量记忆的第一道闸是**克制写入**:宁可漏写,不写脏。决定"何时算定型、何时触发"的是上层(策略);模块只提供能力(机制)。

**召回时机(选择性,不每轮全量)**:每轮把所有相关记忆灌进 context 会因 Lost-in-the-Middle / 近似干扰项 / context rot 而损害精确率(证据见 [retrieval §选择性召回](./retrieval.md#选择性召回recallrouter-与-memory-as-a-tool))。因此召回前先过门控(要不要召回 / 召回哪类 / 取多少)。触发权在应用层,模块提供可插拔的 `RecallRouter`(MVP 薄启发式)与 memory-as-a-tool 暴露形态——详见 retrieval。

## 单/多用户作用域与 owner 语义(ADR 0009)

**单用户是多用户的退化情形,不做两套设计。** 单机单用户时所有记忆落在 `(namespace=default, owner_id=该用户)`;多用户只是这两个字段取不同值(与"基座+垂直分层"同构)。

把两件不同性质的事分开([ADR 0009](../../adr/0009-single-multi-user-scoping-isolation.md)):

- **隔离 = 机制(现在做)**:用户 A 永远看不到 B 的记忆。检索强制带 `(namespace, owner_id)` prefilter,缺作用域则拒绝(fail-closed),作用域从可信上下文派生。详见 [retrieval §作用域隔离](./retrieval.md#作用域隔离每次检索必带作用域缺则拒绝)。
...
| kind | owner | 可共享性 | 本阶段 |
|------|-------|---------|--------|
| semantic | `user_id` | 私有("领域/团队事实"不是记忆,归上下文模块,见 ADR 0006) | 仅私有 |
| episodic | `user_id` | 私有(情景天然个人专属) | 仅私有 |
| procedural | **二选一** | 私有 `f(agent_id,user_id)` / 全局技能 `agent_id`(namespace 内跨用户) | **仅实现私有**;全局留接口位 |
...
> **不做(YAGNI)**:组织/全局三层继承式检索(业界无成熟范式)、跨用户脱敏共享池(依赖 ADR 0008 pipeline,阶段二)。但 `namespace` 层级化前缀(未来 `org:school_a`)不堵死。
...
class MemoryBase(LanceModel):
    ...
    # ---- 血缘 ----
    source_trace_id: str | None = None  # 来源 trace(procedural 必填)
```
...
不记什么:发生过的对话/事件本身(那是 episodic 记忆);Agent 自己的执行策略(那是 procedural);世界通用知识/领域资料(教材、课标——归上下文模块,见 §垂直化)。**边界清晰是低噪音的前提**——记错了类别,检索时就会污染结果。
...
2. **从对话抽取**:本阶段**最小可用**版本——提供 `extract_semantic(messages) -> list[SemanticMemory]` 钩子,单次 LLM 调用从一段对话抽取候选事实/偏好。**默认不自动跑**,由上层在合适时机(如会话结束)显式触发。

> **取舍**:不做后台自动抽取与持续演进(Non-goal)。自动抽取易写入噪声、产生冲突,需配套去重/反思机制才稳妥,成本高。本阶段把"何时抽取"的决策权交给上层,infra 只提供能力。

### 检索路径

默认 `method=hybrid`(向量 + BM25 + RRF),按 `owner_id`/`namespace` 过滤、`deprecated=False`;可选 `category` 过滤。命中后异步更新 `last_used_at`,不阻塞返回。语义记忆是检索的**主精确率层**——干净、去重、可信。
...
- **显著性门控下追加**:每轮对话/工具调用产生的、通过显著性门控的内容,由适配层在会话推进时写入。`salience` 由门控打分。
- **追加式,不做 LLM 冲突决策**(事件有时序意义,不像事实需要去重/取代)。
...
3. **可选沉淀**:可触发 `extract_semantic`,把情景里值得长期保留的事实晋升为 `semantic`(本阶段提供钩子,默认不自动跑)。
...
class ProceduralMemory(MemoryBase):
    ...
    source_trace_id: str          # 必填:来源 trace(回指原始,兜底回溯)
```

`text`/`text_tokens`/`vector` 由 `situation`(+可选 `task_type`)生成——**检索主要按"情况"匹配**,因为 Agent 检索经验是为回答"我现在的情况,以前怎么处理"。

### 经验从哪来:评估/提炼在模块外(ADR 0008)

**关键边界:把一条原始执行 trace 评估、提炼成"值得记的经验",是质量评估(策略),不是存储能力(机制)。** 因此这条链路**不在记忆模块内**——记忆模块对 procedural 只暴露"写入一条已提炼、已评估的经验"(`write_experience`,见 [api](./api.md)),所有"决定记什么经验"的智能在模块外。

```mermaid
flowchart LR
    T["原始 Agent 执行 trace<br/>(思考/工具调用/结果/错误)"]
    subgraph OUT["【模块外:评估/提炼 pipeline】(ADR 0008)"]
        E1["采集 + 评估成败"]
        E2["提炼可复用模式<br/>(situation/action/outcome)"]
        E3["跨执行聚合 / 可选人工审核"]
    end
    W["记忆模块写接口<br/>write_experience()"]
    DB["procedural 库<br/>(去重 + 衰减排序 + 检索)"]
    FB["reinforce():复用反馈记账"]
    T --> E1 --> E2 --> E3 --> W --> DB
    DB --> FB
    FB -.提升/降低.-> DB
...
- **trace 抽象在底座**(`foundation/tracing.py`,横切能力,可观测性与提炼共用);**采集/评估/提炼在模块外**;**去重/写入/衰减/检索/强化记账在记忆模块内**(机制)。三段边界见 [ADR 0008](../../adr/0008-procedural-evaluation-decoupling.md)。
- **保留原始 trace 兜底**:`source_trace_id` 回指原始 trace,提炼丢了的可回溯;且提炼在模块外、可独立改进、可重跑,不是一次性焊死(缓解 *Storage Is Not Memory* 的摄入端加工风险)。
- **评估不盲信单次 LLM 打分**:LLM-as-judge 有偏置与不稳定(arXiv 2412.12509 等),模块外评估要么多次采样、要么以真实执行结果(success 信号)兜底。

> **MVP 占位(YAGNI)**:本阶段**不真建评估平台**。给一个**模块外**的最小占位生产者——规则门控(步数过短 / 与近期 trace 雷同则跳过)+ 单次 LLM 抽取 `situation/action/outcome` + 简单初始 `effectiveness`(成功 0.6 / 失败 0.3),产物经 `write_experience` 落库。**最终形态待定**(外接 observability 平台如 LangSmith/Langfuse/Phoenix,或后续独立 Kairos 模块),本阶段不绑定。

### 复用反馈(reinforce,机制)

一条经验被检索命中、被 Agent 采用且本次执行成功时,上层回调 `reinforce(experience_id, success=True)`,提升 `effectiveness`/`reuse_count`;反之降低(简单指数移动平均,复杂信用分配列为扩展)。**这是程序记忆区别于另两类的关键——它会"越用越准"。** 记账逻辑是机制(在模块内);"何时调用"由上层决定(策略)。
...
|------|--------|---------|
| 三类记忆 CRUD + 检索 + 淘汰 | ✅ | — |
| 工作记忆(context 压缩) | ✅ 归应用/适配层(ADR 0006),infra 不存 | infra 统一调度(MemGPT 式 paging) |
| 写入冲突决策(semantic/procedural) | ✅ LLM 驱动 ADD/UPDATE/DELETE/NOOP | 语义聚类去重 |
| episodic 显著性门控 | ✅ 规则门控 | LLM/学习式显著性打分 |
| 从对话抽取语义记忆 | ✅ 钩子,信息定型时由上层触发 | 自动抽取 + 冲突消解 |
| trace 评估/提炼程序记忆 | ✅ **模块外**最小占位生产者(规则门控 + 单次 LLM 抽取);模块只收已提炼经验(ADR 0008) | 独立评估 pipeline / observability 平台 / 自动分段、反思循环、信用分配 |
| 召回时机门控 | ✅ RecallRouter 薄启发式 + memory-as-a-tool(ADR 0007) | LLM/训练式路由(Self-RAG/Adaptive-RAG/UAR) |
| 作用域隔离 | ✅ 强制 (namespace,owner_id) prefilter + fail-closed + 契约测试(ADR 0009) | 完整 RBAC/鉴权/配额 |
| 多用户记忆共享 | ❌ 本阶段仅私有(ADR 0009) | procedural 全局技能池 + 去标识脱敏(随 ADR 0008 pipeline,阶段二) |
