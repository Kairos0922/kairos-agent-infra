...
**TenantContext 显式传参,不用 contextvar(ADR 0012)**:`ctx: TenantContext` 作为每个读写/检索接口的显式首参,而非藏进 contextvar 隐式全局态。理由=**契约可见**(签名即声明"这是租户隔离的操作")、**可被契约测试覆盖**(测试直接构造 ctx 传入)、**无隐式全局态**(避免 async 上下文传播的隐蔽 bug 与跨请求串味)。代价是签名更长,可接受。
...
| 对外 API 形态 | 进程内 async Python 库(模块契约) | MVP 单机,零网络开销;server 层预留服务化 | 直接上 HTTP 服务(过早) |
