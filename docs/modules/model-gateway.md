...
花了多少钱"。骨架:contracts/ + providers/ + router/ + factory.py。
...
class ChatModel(Protocol):
    ...
    # ChatChunk: text_delta | tool_call_delta | usage | stop

class EmbeddingModel(Protocol):
    async def embed(self, texts: list[str]) -> list[Vector]

class RerankModel(Protocol):
    async def rerank(self, query: str, docs: list[str]) -> list[Score]

class Tokenizer(Protocol):
    def count(self, text: str) -> int          # 纯 CPU,同步(既定约定)
```
...
- 结构化输出:response_schema(JSON Schema)为一等参数;
  ...
  +一次修复重试"的兜底(上层无感知)。
- 原始异常(openai/anthropic/httpx)一律封装 ProviderError
...
- 降级链:primary 重试耗尽 → fallback 按序;全链失败才向上抛
  ProviderError(loop 据此 FINISHED(failed),S4 已定)。
...
class UsageSink(Protocol):
    async def record(self, ctx: TenantContext, rec: UsageRecord) -> None
    # UsageRecord: run_id/agent_path/tier/实际模型/in・out tokens/cost/ts

class UsageQuery(Protocol):
    async def aggregate(self, ctx, window) -> UsageAggregate  # server 配额消费
```
...
- **候选 A(推荐):契约归模块不动,组装根适配。**
  ...
  ✔ import-linter 全绿:适配器住 server/composition/,
    只有组装根同时 import 两个模块。
