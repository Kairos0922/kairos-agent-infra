...
## 追记(2026-07-07,ADR 0023)

本 ADR 结论「user_id 由客户端在请求体声明」已由 [ADR 0023](./0023-user-level-authentication.md) 修订为「**user_id 由认证结果派生,不接受客户端自由声明**」——机构租户内同校用户(教师)间隔离是产品命脉与未成年人 PII 合规刚需,须认证。API Key per tenant、`Authenticator` 契约、`TenantContext` 唯一构造点结论不变;用户级认证是 `Authenticator` 的新实现,正落在本 ADR 已留的扩展位上。
