...
本 ADR 曾把同一 Runtime 同时服务"本地 C 端个人 + 服务器"列为并列诉求。目标形态明确后(1000+ 用户、教育机构租户),**主部署目标定为机构云**(见 [ADR 0022](./0022-deployment-topology-cell-per-tenant.md) 的 cell-per-tenant);本地端为**同一实现的另一构建档(build profile)**,按需切换存储后端与 panic 策略,而非用一个二进制的默认档同时满足两端。据此 workspace `[profile.release]` 默认档改为服务器取向:`panic = "unwind"`(避免单次 panic 打穿整个多用户 cell)、`opt-level = 3`(取吞吐,替代原本地端的体积优先 `"z"`);本地小体积档待真有本地分发需求时再加(YAGNI)。语言分工结论不变。
