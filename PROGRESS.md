...
范围与设计依据见 [docs/project/roadmap.md](./docs/project/roadmap.md)。设计研讨结论见 ADR 0004(不做图)、0005(衰减/删除分离)、0006(认知功能分类)、0007(机制/策略分离与选择性召回)、0008(procedural 评估/提炼解耦)、0009(单/多用户作用域与隔离)与 [benchmark 子项目](./docs/modules/benchmark/README.md)。
...
- [ ] 作用域隔离:强制 (namespace,owner_id) prefilter + fail-closed + 跨 owner 隔离契约测试(ADR 0009)
...
- 2026-06-27 单/多用户记忆设计研讨定案:新增 ADR 0009(单/多用户作用域与隔离)。确立"单用户=多用户退化、隔离是机制(现在做)vs 共享是策略(阶段二)";两正交轴复用 namespace(租户硬边界)+ owner_id(实体),不加新字段;隔离三规则——强制 (namespace,owner_id) prefilter、fail-closed(缺作用域则拒绝)、作用域从可信上下文派生不信自报;三类记忆 owner 语义(episodic/semantic 私有,procedural 私有/全局二选一,本阶段仅私有,全局共享依赖 ADR 0008 脱敏 pipeline);跨 owner 隔离落契约测试(对应 OWASP LLM02/08)。经联网核源(Mem0 entity-scoped/LangGraph namespace/Letta block、LanceDB/Qdrant/Pinecone/Weaviate 多租户、OWASP 2025、Voyager/ExpeL/AWM;2026 preprint 标待检验)。同步:新增 ADR 0009 + 索引;改 memory-types/retrieval/api/tradeoffs/foundation/overview/memory README/根 README/PROGRESS。
